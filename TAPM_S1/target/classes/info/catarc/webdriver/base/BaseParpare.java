package info.catarc.webdriver.base;
/**
 * @author Guojs
...
import info.catarc.webdriver.utils.ExcelDataProvider;
import info.catarc.webdriver.utils.LogConfiguration;
...
public class BaseParpare {
	...
	public void startTest(ITestContext context) {
		...
		//userName = context.getCurrentXmlTest().getParameter("testurl");
		//password = context.getCurrentXmlTest().getParameter("000000");
	...
	public Iterator<Object[]> dataFortestMethod() throws IOException {
		...
		logger.info("当前测试类名为:[" + className + "]");
		int dotIndexNum = className.indexOf("."); // 取得第一个.的index
		int underlineIndexNum = className.indexOf("_"); // 取得第一个_的index

		if (dotIndexNum > 0) {	
			moduleName = className.substring(22, className.lastIndexOf(".")); // 取到模块的名称
